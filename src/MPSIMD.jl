...
function normalize1(::Type{I}, x::Vec{N, F}) where {F, I, N}
    nbits = 8 * sizeof(I)
...
# function Base. *(x::IntN{F, I, N}, y::Integer)::IntN{F, I, N} where {F, I, N}
#     x * I(y)
# end
...
function Base. *(x::IntN{F, I, N}, y::I)::IntN{F, I, N} where {F, I, N}
    nbits = 8 * sizeof(I)
    nbits2 = nbits ÷ 2
    mask2lo = I((I(1) << nbits2) - 1)
    mask2hi = ~mask2lo
    ylo = y & mask2lo
    yhi = y & mask2hi
    rlo = x.digits * F(ylo)
    rhi = normalize1(I, x.digits * F(yhi))
    IntN(Internal{F, I, N}(), normalize1(I, rlo + rhi))
end
